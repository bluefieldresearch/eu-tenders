-- Desalination O&M contracts in Spain + Catalonia with company groups
-- Combines Spanish (PLACE) and Catalan (Transparencia Catalunya) data
-- Export to CSV: \copy (...) TO 'exports/desalination_spain_cat.csv' WITH CSV HEADER
...
AND (objecte_contracte ILIKE '%dessalinitz%'
   OR objecte_contracte ILIKE '%dessaladora%'
   OR objecte_contracte ILIKE '%ITAM %'
   OR objecte_contracte ILIKE '% ITAM%')
...
AND objecte_contracte NOT ILIKE '%subministrament%'
